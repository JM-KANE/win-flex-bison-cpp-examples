...
#line 53 "simple.y"
            {
                for (auto& str : yystack_[0].value.as < std::vector<std::string> > ())
                {
                    std::cout << str << ' ';
                }
                std::cout << '\n';
...
#line 625 "simple.cpp"
  ...
  case 4: // @1: %empty
#line 64 "simple.y"
                      { yylhs.value.as < std::size_t > () = yystack_[0].value.as < std::vector<std::string> > ().size(); }
#line 631 "simple.cpp"
    break;

  case 5: // list: list @1 item
#line 64 "simple.y"
                                               { yylhs.value.as < std::vector<std::string> > () = yystack_[2].value.as < std::vector<std::string> > (); yylhs.value.as < std::vector<std::string> > ().emplace_back (std::move(yystack_[0].value.as < std::string > ())); }
#line 637 "simple.cpp"
    break;
...
#line 653 "simple.cpp"
  ...
  "$accept", "result", "list", "@1", "item", YY_NULLPTR
  };
#endif
