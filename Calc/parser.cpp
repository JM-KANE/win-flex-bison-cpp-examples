...
#line 53 "parser.y"
                      { drv.result = yystack_[0].value.as < Node* > (); }
#line 614 "parser.cpp"
    break;

  case 5: // assignment: "identifier" ":=" exp
#line 61 "parser.y"
                          { drv.variables[yystack_[2].value.as < std::string > ()] = driver::VariableInfo{ yystack_[2].location,yystack_[0].value.as < Node* > () }; }
#line 620 "parser.cpp"
    break;
...
#line 65 "parser.y"
                    { yylhs.value.as < Node* > () = drv.variables[yystack_[0].value.as < std::string > ()].value->Copy(); }
#line 632 "parser.cpp"
    break;

  case 8: // exp: exp "+" exp
#line 66 "parser.y"
                    { yylhs.value.as < Node* > () = yystack_[2].value.as < Node* > (); yylhs.value.as < Node* > ()->value += yystack_[0].value.as < Node* > ()->value; delete yystack_[0].value.as < Node* > (); }
#line 638 "parser.cpp"
    break;
